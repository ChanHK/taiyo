...
if ($conn) {
    ...
    /*$sql = "CREATE TABLE review (review_id INT(6) UNSIGNED AUTO_INCREMENT NOT NULL, review_message VARCHAR(255), review_date DATE, 
			reviewer_id INT(6) UNSIGNED,
			reviewee_id INT(6) UNSIGNED,
			CONSTRAINT FK_REVIEWER FOREIGN KEY(reviewer_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE,
			CONSTRAINT FK_REVIEWEE FOREIGN KEY(reviewee_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE,
			PRIMARY KEY (review_id))";
	*/

    /* $sql = "CREATE TABLE product (product_id INT(6) UNSIGNED AUTO_INCREMENT NOT NULL, product_name VARCHAR(30), 
				product_price FLOAT(7,2), 
				product_description VARCHAR(255), 
				product_state VARCHAR(50), 
				category VARCHAR(50), 
				quantity INT(2), 
				enduser_id INT(6) UNSIGNED,
				CONSTRAINT FK_PRODUCTUSER FOREIGN KEY(enduser_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE,
				PRIMARY KEY (product_id))";
	*/

    /* $sql = "CREATE TABLE cart (cart_item_id INT(6) UNSIGNED AUTO_INCREMENT NOT NULL, quantity INT(2), 
				product_id INT(6) UNSIGNED, 
				enduser_id INT(6) UNSIGNED, 
				CONSTRAINT FK_CARTPRODUCT FOREIGN KEY(product_id) REFERENCES product(product_id) ON DELETE CASCADE ON UPDATE CASCADE, 
				CONSTRAINT FK_CARTUSER FOREIGN KEY(enduser_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE, 
				PRIMARY KEY (cart_item_id))";
				
	*/
	
    /* $sql = "CREATE TABLE productimage (product_image_id INT(6) UNSIGNED AUTO_INCREMENT NOT NULL, 
				product_id INT(6) UNSIGNED, 
				product_image VARCHAR(255), 
				CONSTRAINT FK_PRODUCTIMAGEPROD FOREIGN KEY(product_id) REFERENCES product(product_id) ON DELETE CASCADE ON UPDATE CASCADE,
				PRIMARY KEY (product_image_id))";
	*/

    /* $sql = "CREATE TABLE wishlist (wishlist_item_id INT(6) UNSIGNED AUTO_INCREMENT NOT NULL, 
				product_id INT(6) UNSIGNED, 
				enduser_id INT(6) UNSIGNED,
				CONSTRAINT FK_WISHLISTPROD FOREIGN KEY(product_id) REFERENCES product(product_id) ON DELETE CASCADE ON UPDATE CASCADE, 
				CONSTRAINT FK_WISHLISTUSER FOREIGN KEY(enduser_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE, 
				PRIMARY KEY (wishlist_item_id))";
	*/
	
	
    /* $sql = "CREATE TABLE transactionhistory (transaction_id INT(6) UNSIGNED AUTO_INCREMENT NOT NULL, transaction_type VARCHAR(10), quantity INT(2),
				product_id INT(6) UNSIGNED,
				belonginguser_id INT(6) UNSIGNED, 
				boughtuser_id INT(6) UNSIGNED,
				CONSTRAINT FK_TRANSPROD FOREIGN KEY(product_id) REFERENCES product(product_id) ON DELETE CASCADE ON UPDATE CASCADE, 
				CONSTRAINT FK_TRANSBELONGUSER FOREIGN KEY(belonginguser_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE, 
				CONSTRAINT FK_TRANSBOUGHTUSER FOREIGN KEY(boughtuser_id) REFERENCES enduser(enduser_id) ON DELETE CASCADE ON UPDATE CASCADE, 
	...
	*/
    ...
    // $sql = "INSERT INTO enduser (user_email, user_password, username, user_description, gender, phone_number, street_1, street_2, city, c_state, postcode, age, profile_photo) VALUES ('Hatiuser2@gmail.com', 'hatiuser1','hatiuser1','Likes to scam people','Female','017-1103490','No. 701, Jalan B', 'Kearny', 'George Town', 'Sabah', '90000',36, 'profile2.jpg')";
	...
	// $sql = "INSERT INTO transactionhistory (transaction_id, transaction_type, quantity, product_id, belonginguser_id, boughtuser_id) VALUES(NULL, 'Sold', 2, 4, 3, 1)";
